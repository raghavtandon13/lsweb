...
export default function ApplicationsPage() {
    ...
    return (
        <div>
            <DashHead body="Every file opened for this mobile, including closed ones." title="My applications" />

            {ready && apps.length === 0 && (
                <div className="card mt-6 p-5 sm:p-6">
                    <p className="font-serif text-xl text-navy">No applications on this mobile</p>
                    <p className="mt-2 text-sm leading-6 text-muted">
                        Logout clears the previous customer. Start a new check with a different number, or build credit
                        first.
                    </p>
            ...
            {apps.length > 0 && (
                <div className="mt-6 hidden overflow-x-auto rounded-2xl border border-line bg-white md:block">
                    <table className="w-full min-w-[640px] text-left text-sm">
                        ...
                        <tbody>
                            {apps.map((a) => (
                                <tr className="border-t border-line" key={a.id}>
