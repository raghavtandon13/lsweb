...
const SUDHAR_WEEKS = [
    {
        w: "Week 1",
        t: "Clear overdue",
        d: "Pay the oldest late dues first. One missed month hurts more than utilisation.",
    },
    {
        w: "Week 2",
        t: "Cut cards to 30%",
        d: "Pay revolving balances before the statement date. Do not close the oldest card.",
    },
    {
        w: "Week 3",
        t: "Stop shopping credit",
        d: "No new hard applications. Soft matching on LoanSparrow does not add a pull.",
    },
    {
        w: "Week 4",
        t: "One on-time cycle",
        d: "Autopay minimum + extra to principal. Recheck score after the bureau cycle.",
    },
];
        ...
        <div className="space-y-4">
            ...
            {addon.slug === "sudhar" && (
                <ol className="space-y-3">
                    {SUDHAR_WEEKS.map((x) => (
                        <li className="card p-4 sm:p-5" key={x.w}>
                            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-spark-gold">
                                {x.w}
                            </p>
            ...
            {addon.slug === "builder" && (
                <div className="card p-5 sm:p-6">
                    ...
                    <p className="mt-4 text-sm leading-6 text-muted">
                        Amount stays parked. On-time reporting is what thickens a thin file like{" "}
                        {apply.name?.split(" ")[0] ?? "this"}’s.
                    </p>
                </div>
            )}
            ...
            {addon.slug === "report" && (
                <>
                    ...
                    <div className="space-y-3">
                        ...
                        {(r.issues ?? []).map((issue) => (
                            <article className="card p-4 sm:p-5" key={issue.title}>
                                <span
                                    className={cn(
            ...
            <ButtonLink className="w-full" href="/apply/cibil" variant="outline">
                Back to score
            </ButtonLink>
            <Link className="block text-center text-sm font-semibold text-spark-gold" href="/apply/addons">
                Other add-ons
            </Link>
        </div>
    );
