...
export default function CibilDetailsPage() {
    ...
    const factors = [
        {
            ...
            value: `${report.paymentHistory}%`,
            bar: report.paymentHistory,
            hint: "On-time EMIs and card dues.",
        },
        {
            label: "Credit utilisation",
            value: `${report.utilisation}%`,
            bar: Math.max(8, 100 - report.utilisation),
            hint: "Keep revolving balances under ~30%.",
        },
        {
            label: "On-time EMIs",
            value: `${Math.round(report.onTime * 100)}%`,
            bar: Math.round(report.onTime * 100),
            hint: "Missed payments stay on file.",
        },
        {
            label: "Enquiries (90 days)",
            value: String(report.enquiries90d),
            bar: Math.max(12, 100 - report.enquiries90d * 18),
            hint: "This matching check is soft — it will not affect your score.",
        },
    ];
    ...
    return (
        <div className="space-y-4">
            <EditDetailsBar />
            <Link className="inline-flex items-center gap-1 text-sm font-semibold text-spark-gold" href="/apply/cibil">
            ...
            <div className="card p-4 sm:p-6">
                ...
                <div className="mt-4 flex flex-col items-center gap-5 sm:flex-row sm:items-center">
                    ...
                    <div className="w-full text-center sm:text-left">
                        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-spark-gold">
                            {report.band}
                        </p>
                        ...
                        <div className="mt-4">
                            ...
                            <div className="relative mt-1 h-1.5 rounded-full bg-line">
                                <div
                                    className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-spark-gold"
                                    style={{ left: `${pct}%` }}
                                />
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
                {[
                    ...
                    <div className="rounded-2xl border border-line bg-white px-3 py-3 sm:px-4" key={k}>
                    ...
                    {factors.map((f) => (
                        <li key={f.label}>
                            ...
                            <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-line">
                                <div
                                    className="h-full rounded-full bg-spark-gold"
                                    style={{ width: `${Math.min(100, f.bar)}%` }}
                                />
                            </div>
            ...
            <div className="card p-4 sm:p-6">
                ...
                <ul className="mt-2 divide-y divide-line">
                    {report.accountsList.map((a) => (
                        <li className="flex items-center justify-between gap-3 py-3 text-sm" key={a.name}>
            ...
            {(report.issues ?? []).length > 0 && (
                <div className="space-y-3">
                    ...
                    {(report.issues ?? []).map((issue) => (
                        <article className="card p-4" key={issue.title}>
